...
mod tests {
    ...
    macro_rules! sha_test {
        ($i:literal) => {
            let s = $i;
            let my_res = super::Sha384::new().chain_update(s.as_bytes()).finalize();
            let ex_res = sha2::Sha384::new().chain_update(s.as_bytes()).finalize();
            assert_eq!(
                ex_res, my_res,
    ...
    fn rand() {
        ...
        for _ in 0..10 {
            ...
            let my_res = super::Sha384::new().chain_update(&buffer).finalize();
            let ex_res = sha2::Sha384::new().chain_update(&buffer).finalize();
